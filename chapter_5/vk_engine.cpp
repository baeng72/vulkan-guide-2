...
void VulkanEngine::init(){
    ...
    glfwSetKeyCallback(pwindow, [](GLFWwindow* window, int key, int scancode, int action, int mods) {
        ...
        bool repeat = action == GLFW_REPEAT ? true : false;
        VulkanEngine*peng=(VulkanEngine*)glfwGetWindowUserPointer(window);
        peng->mainCamera.processKey(key,scancode,pressed, repeat);
    ...
    glfwSetCursorPosCallback(pwindow, [](GLFWwindow* window, double xpos, double ypos) {
            ...
            float xrel = (fxpos - peng->oldXPos);
            float yrel = (fypos - peng->oldYPos);
            peng->mainCamera.processMouseMove(xrel, yrel);
            peng->oldXPos = fxpos;
            peng->oldYPos = fypos;            
        });
    ...
    oldXPos = _windowExtent.width / 2.f;
    oldYPos = _windowExtent.height / 2.f;
    mainCamera.velocity = vec3(0.f);
    mainCamera.position = vec3(0.f, 0.f, 5.f);
...
bool is_visible(const RenderObject & obj, const mat4& viewproj){
    ...
    for(i32 c = 0; c < 8; ++c){
        ...
        min = glm::min(vec3(v),min);
        max = glm::max(vec3(v),max);
    }
   ...
   VkBuffer lastIndexBuffer = VK_NULL_HANDLE;
    auto draw = [&](const RenderObject&r){
        if(r.material != lastMaterial){   
            ...
            //rebind pipeline and descriptors if the material has changed
        ...
        //add counters for trianles and draws
        stats.drawcall_count++;
        stats.triangle_count += r.indexCount / 3;
        
    };

    for(auto&r : opaque_draws){//mainDrawContext.OpaqueSurfaces){
...
void VulkanEngine::run(){
    ...
    oldXPos = _windowExtent.width / 2.f;
    oldYPos = _windowExtent.height / 2.f;
    glfwSetCursorPos(pwindow, oldXPos, oldYPos);
    ...
    while(_isRunning){
        ...
        if(stop_rendering){
            ...
            ImGui::Text("draw %i", stats.drawcall_count);
            ImGui::End();
        ...
        //convert to microseconds (integer), and then com back to milli
        auto elapsed = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
